...
public class BoxLayer3_CarAd
  extends SYSprite
{
  String ad_word_url;
  
  protected BoxLayer3_CarAd(Texture2D paramTexture2D, String paramString)
  {
    super(paramTexture2D);
    setAnchor(0.0F, 0.0F);
    if (Const.BASE_WIDTH == 1280) {
      setPosition(59.0F, 220.0F);
    }
    for (;;)
    {
  ...
  public void launchLenovoMarket(String paramString)
  {
    ...
    localActivity.runOnUiThread(new Runnable()
    {
      public void run()
      {
        Toast.makeText(Director.getInstance().getContext(), BoxLayer3_CarAd.this.getInfoByLanguage(), 1).show();
      }
    });
  }
  ...
  public boolean wyTouchesEnded(MotionEvent paramMotionEvent)
  {
    ...
    if (hitTest(paramMotionEvent.x, paramMotionEvent.y))
    {
      ...
      paramMotionEvent.put("adword", "车开动时的图片广告");
      ...
      TCAgent.onEvent(Director.getInstance().getContext(), "Car_Ad_click", this.ad_word_url);
