...
public class DatabaseUpdateTask extends AsyncTask<Void, Void, Boolean> {
	...
	protected Boolean doInBackground(Void... params) {	    				
		// Déclaration de variables avant la boucle
		...
		// Vide les tables concernées dans la base de données
    	...
    	// Parcourt chaque package du système
    	for (ApplicationInfo ai : packages)
    	{
    		// Incrémente la barre de progression
    		progressDialog.incrementProgressBy(1);
    		
    		// Récupère le nom du package et si possible le label
        	...
        	// Récupère si possible les versions
        	try {
        		 ...
        		 if (pi.requestedPermissions != null && pi.requestedPermissions.length > 0) {
        			 ...
        			 for (int i = 0; i < pi.requestedPermissions.length; ++i) {
        				 if (pi.requestedPermissions[i].startsWith("android.permission.")) {
        					 // si c'est une permission officielle d'android, on récupère son nom
        					 ...
        					 // On récupère l'id de la permission
	...
	protected void onPostExecute(Boolean result) {
		// Fermeture de la barre de progression et appel de la fonction de résultat
