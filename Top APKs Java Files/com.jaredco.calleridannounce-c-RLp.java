...
public class RLp
{
  private static final String ˊ = RLp.class.getSimpleName();
  ...
  private ArrayList<String> ˊ(Context paramContext)
  {
    try
    {
      ...
      while (((Iterator)localObject).hasNext())
      {
        ...
        RRw.ˊ(ˊ, "get Local Package: " + localApplicationInfo.packageName);
      }
      return paramContext;
    }
    catch (Exception paramContext)
    {
      RRw.ᐝ(ˊ, "Using fallback package getter");
      paramContext.printStackTrace();
      return ˊ();
    }
  }
  
  public void ˊ(Context paramContext, RsJ paramRsJ)
  {
    ClientConfig localClientConfig = RRJ.ˊ(paramContext).ˋ();
    ...
    Rsg localRsg = localClientConfig.ᐪ();
    if (localRsg == null) {
      localRsg = new Rsg();
    }
    for (;;)
    {
      ...
      Iterator localIterator1 = paramRsJ.ˋ().iterator();
      label83:
      label515:
      for (;;)
      {
        ...
        {
          localRsw = (Rsw)localIterator1.next();
          ...
          } while (!((String)localIterator2.next()).equalsIgnoreCase(localRsw.ˊ()));
        }
        for (int i = 1;; i = 0)
        {
          if ((i != 0) || (localRsw.ˊ().equals(paramContext.getPackageName()))) {
            break label515;
          }
          localArrayList.add(localRsw.ˊ());
          break label83;
          i = 1;
          if (l1 == 0L) {
            i = 0;
          ...
          while (i != 0)
          {
            ...
            paramRsJ = paramRsJ.ˋ().iterator();
            while (paramRsJ.hasNext())
            {
              localObject = (Rsw)paramRsJ.next();
              if ((!((Rsw)localObject).ˊ().equals(paramContext.getPackageName())) && (((Rsw)localObject).ˏ() != 0L) && (l3 - ((Rsw)localObject).ˏ() > l1))
              {
                localArrayList.add(((Rsw)localObject).ˊ());
                RRw.ᐝ(ˊ, "Client timeout -  pack.getName()" + ((Rsw)localObject).ˊ() + ", currentTime " + l3 + ", pack.getTimeStamp(): " + ((Rsw)localObject).ˏ() + ", MAX_TIMEOUT: " + l1);
              }
            }
            if (l3 - l2 > l1)
            {
              RRw.ˊ(ˊ, "over maxtimeout, cant determine dead clients");
              localClientConfig.ˋ(l3);
              if ((paramRsJ != null) && (paramRsJ.ˋ() != null))
              {
                localObject = paramRsJ.ˋ().iterator();
                while (((Iterator)localObject).hasNext()) {
                  ((Rsw)((Iterator)localObject).next()).ˊ(l3);
                }
                localClientConfig.ˊ(paramRsJ);
              }
              i = 0;
            }
          }
          i = 0;
          if (localArrayList.size() != localRsg.ˊ().size()) {
            i = 1;
          }
          while (i != 0)
          {
            localRsg.ˊ(localArrayList);
            localRsg.ˊ(localRsg.ˋ() + 1);
            localClientConfig.ˊ(localRsg);
            return;
            paramContext = new ArrayList(localArrayList);
            paramContext.removeAll(localRsg.ˊ());
