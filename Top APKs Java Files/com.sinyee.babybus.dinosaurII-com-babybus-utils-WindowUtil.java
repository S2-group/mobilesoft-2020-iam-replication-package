...
public class WindowUtil
{
  ...
  public static void launchApp(Activity paramActivity, String paramString, boolean paramBoolean)
  {
    ...
    if (localObject != null)
    {
      ...
      {
        ShellCmdBuilder.get().setAction("android.intent.action.MAIN").addCategory("android.intent.category.LAUNCHER").addFlags(268435456).setComponent(paramString, (String)localObject).addPram("launchFlag", Boolean.valueOf(true)).execShellCmd();
      }
      else
      {
      ...
      paramActivity.runOnUiThread(new Runnable()
      {
        public void run()
        {
          ...
          if (WindowUtil.hasAnyMarketInstalled(paramActivity))
          {
            if (ShellCmdBuilder.isCmdStartActivity())
            {
              ShellCmdBuilder.get().setAction("android.intent.action.VIEW").addFlags(268435456).setDataUri(((Uri)localObject).toString()).execShellCmd();
              return;
            }
