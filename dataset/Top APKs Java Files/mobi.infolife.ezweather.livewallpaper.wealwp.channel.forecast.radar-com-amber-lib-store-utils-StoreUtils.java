...
public class StoreUtils
{
  ...
  public static void applyWidgetByPkgNameWithoutStartApplyWidget(Context paramContext, String paramString)
  {
    String str = WidgetStatusManager.getInstance().getMainWidget();
    Object localObject = str;
    if (!ToolUtils.checkAppInstalled(paramContext, str)) {
      localObject = paramContext.getPackageName();
    }
    if (TextUtils.equals(paramContext.getPackageName(), (CharSequence)localObject)) {
      WidgetStatusManager.getInstance().setUsingTheme(paramString);
    }
    localObject = new Intent();
    ((Intent)localObject).setComponent(new ComponentName(paramContext.getPackageName(), "com.amber.lib.store.activity.ApplyStoreWidgetActivity"));
    ((Intent)localObject).putExtra("theme_package_name", paramString);
    ((Intent)localObject).putExtra("is_pro_version", true);
  ...
  {
    StoreDataRequest.downloadData(StoreDataRequest.getVersionUrl(paramContext), new StoreUtils.1(paramContext));
  }
}
