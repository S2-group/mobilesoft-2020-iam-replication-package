...
import com.tabtale.publishingsdk.core.utils.PublishingSDKAppLoggerPriority;
...
public class AppShelfServiceImpl
  implements AppLifeCycleDelegate, LocationInternalDelegate, AppShelfService
{
  ...
  private static final String JSON_CRC32 = "crc32";
  ...
  protected AppShelfServiceImpl(AppLifeCycleMgr paramAppLifeCycleMgr, String paramString1, String paramString2, String paramString3, String paramString4, String paramString5, PublishingSDKAppInfo paramPublishingSDKAppInfo, LocationInternalDelegate paramLocationInternalDelegate)
  {
  ...
  private boolean downloadAdditionalResources(JSONObject paramJSONObject, String paramString)
  {
    try
    {
      ...
      while (localIterator.hasNext())
      {
        ...
        long l1 = ((JSONObject)localObject).getLong("crc32");
        localObject = ((JSONObject)localObject).getString("url");
        HttpConnector localHttpConnector = createHttpConnector();
        String str1 = paramString + "/apps/thumbnails";
        String str2 = str1 + "/" + ((String)localObject).substring(((String)localObject).lastIndexOf('/') + 1);
        if (!this.mFileUtils.isFileExist(str2)) {
          if (localHttpConnector.startDownload((String)localObject, str1))
          {
            long l2 = this.mFileUtils.calcFileCRC(str2);
            if (l2 != l1)
            {
              logError("downloadAdditionalResources crc comaparison failed to download file: " + (String)localObject + ", server crc: " + l1 + ", calculated crc: " + l2);
          ...
          {
            logError("downloadAdditionalResources failed to download file: " + (String)localObject);
            this.mLastError = PublishingSDKErrors.PSDK_APPSHELF_FAILED_TO_DOWNLOAD_ADDITIONAL_RESOURCES_ERROR;
            return false;
          }
        }
  ...
  private void downloadAppShelfsResources()
  {
    ...
    while (localIterator.hasNext())
    {
      ...
      if (localAppShelfLocationData.isExist())
      {
        ...
        logApplicationNumber(updateInstallationStatus(localAppShelfLocationData, localSet), localAppShelfLocationData.getLocationName());
    ...
    throws JSONException
  {
    ...
    for (;;)
    {
      ...
      if ((k == 0) && (!downloadThumbnail(str1, localJSONObject.getLong("crc32"), paramString, localJSONObject)))
      {
        k = -1;
        return k;
      }
  ...
  private boolean downloadDefaultAppShelf()
  {
    ...
    Object localObject = createHttpConnector().startDownload(str);
    if ((localObject == null) || (((String)localObject).isEmpty()))
    {
    ...
    this.mDefaultAppShelfExist = downloadFullAppShelfResources(new AppShelfLocationData("default", str, false), refreshInstalledPackages(), (JSONObject)localObject);
    ...
    throws JSONException
  {
    ...
    while (paramList.hasNext())
    {
      ...
      if (!bool)
      {
        if (!downloadThumbnail(localJSONObject.getString("thumbnail"), localJSONObject.getLong("crc32"), paramString, localJSONObject))
        {
  ...
  private boolean downloadThumbnail(String paramString1, long paramLong, String paramString2, JSONObject paramJSONObject)
  {
    HttpConnector localHttpConnector = createHttpConnector();
    paramString2 = paramString2 + "/apps/thumbnails";
    String str = paramString2 + "/" + paramString1.substring(paramString1.lastIndexOf('/') + 1);
    ...
    {
      l = this.mFileUtils.calcFileCRC(str);
      if (l == paramLong)
      {
        addInHouseAdImageToJson(paramJSONObject, str, "thumbnail");
        return true;
      }
      logError("downloadThumbnail crc comaparison failed to download thumbnail file: " + paramString1 + ", server crc: " + paramLong + ", calculated crc: " + l + ", will try again");
    }
    while (localHttpConnector.startDownload(paramString1, paramString2))
    {
      l = this.mFileUtils.calcFileCRC(str);
      if (l == paramLong)
      {
      ...
      {
        logError("downloadThumbnail crc comaparison failed to download thumbnail file: " + paramString1 + ", server crc: " + paramLong + ", calculated crc: " + l);
      }
    }
    for (this.mLastError = PublishingSDKErrors.PSDK_APPSHELF_THUMBNAIL_CRC_ERROR;; this.mLastError = PublishingSDKErrors.PSDK_APPSHELF_FAILED_TO_DOWNLOAD_THUMBNAIL)
    {
      return false;
  ...
  private void logError(String paramString)
  {
    ...
    ServiceManager.instance().getLogger().log(paramString, PublishingSDKAppLoggerPriority.ERROR);
  }
  ...
  public boolean show(String paramString)
  {
    ...
    if (paramString != null) {
      str1 = (String)this.mLocations.get(paramString);
    }
    ...
    String str2 = "default";
    if (str1 != null) {
      if (mReadyLocations.contains(str1)) {
        str2 = str1;
      }
    }
    for (;;)
    {
      str1 = "file:///" + this.mAppShelfDir + str2 + "/skin/index.html";
      this.mShownLocation = paramString;
      this.mWebViewService = new WebViewService(this);
      this.mAppInfo.getActivity().runOnUiThread(new Runnable()
      {
        public void run()
        {
          AppShelfServiceImpl.this.mWebViewService.openWebViewWithUrl(str1, AppShelfServiceImpl.this.mAppInfo.getActivity(), AppShelfServiceImpl.this.mStore);
        }
      });
      return true;
    ...
    extends AsyncTask<Boolean, Void, Void>
  {
    ...
    protected Void doInBackground(Boolean... paramVarArgs)
    {
      ...
      for (;;)
      {
        AppShelfServiceImpl.access$002(AppShelfServiceImpl.this, false);
        return null;
